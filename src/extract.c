...
extract_by_ts(cppip_t *c)
{
    ...
    /** 
     * sanity check:
     * We already know stop ts > start ts, but make sure start ts > first ts.
     */
     ...
     * normalized timestamp while we then divide by the index_level to obtain
     * a multiplier to use to advance to the 
     *
